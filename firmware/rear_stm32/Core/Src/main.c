...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
float live_position3 = 0.0f;
float live_position2 = 0.0f;
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        if (rx_data[2] == 0x33 && rx_data[3] == 80) {
            uint32_t raw = (rx_data[7] << 24) | (rx_data[6] << 16) |
                           (rx_data[5] << 8) | rx_data[4];
            float value;
            memcpy(&value, &raw, sizeof(float));

            // Round to 3 decimals (optional)
            value = ((int)(value * 1000)) / 1000.0f;

            // Send float to Pi over USB
            CDC_Transmit_HS((uint8_t*)&value, sizeof(float));
        }
    }
}
...
int main(void)
{
  ...
  CDC_Transmit_HS((uint8_t*)"STM32 USB READY\n", 17);
...
static void MX_FDCAN1_Init(void)
{
  ...
  FDCAN_FilterTypeDef sFilterConfig;
      ...
      sFilterConfig.FilterID1 = 0x000;  // ID da accettare
      sFilterConfig.FilterID2 = 0x7FF;  // Maschera: accetta solo 0x7FF
...
static void MX_FDCAN3_Init(void)
{
  ...
  hfdcan3.Init.NominalTimeSeg1 = 32;
